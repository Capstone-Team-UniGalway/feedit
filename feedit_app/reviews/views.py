...
def create_review(request, company_id):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            print(f"Parsed data - Rating: {rating}, Content: {content}, Anonymous: {is_anonymous}")

            # Validate rating
            if rating < 1 or rating > 5:
                messages.error(request, 'Rating must be between 1 and 5 stars.')
                return render(request, 'reviews/review_form.html', {'company': company})
...
def create_review_reply(request, review_id):
    ...
    if request.method == 'POST':
        try:
            ...
            if not content.strip():
                ...
                return render(request, 'reviews/review_reply_form.html', {'review': review, 'company': company})
    ...
    return render(request, 'reviews/review_reply_form.html', context)
