...
class TestJoinCompanyView:
    """Test the JoinCompanyView for displaying companies to join."""
    ...
    def test_view_accessible_without_authentication(self):
        """Test that unauthenticated users can access the join company page."""
        response = self.client.get(self.url)
    ...
    def test_view_accessible_with_authentication(self):
        """Test that authenticated users can access the join company page."""
        user = UserFactory()
        self.client.force_login(user)
    ...
    def test_queryset_filters_deleted_companies(self):
        """Test that deleted companies are not shown in the join list."""
        # Create active companies
        active_company1 = CompanyFactory(name="Active Company 1")
        active_company2 = CompanyFactory(name="Active Company 2")
    ...
    def test_template_used(self):
        ...
        assert response.status_code == 200
        assert 'pages/join_company.html' in [t.name for t in response.templates]
    ...
    def test_url_routing(self):
        ...
        response = self.client.get('/join_company')
        assert response.status_code == 200

    def test_different_template_from_public_list(self):
        """Test that JoinCompanyView uses a different template than PublicCompanyListView."""
        response = self.client.get(self.url)

        # Should use join_company.html, not company_list.html
        template_names = [t.name for t in response.templates]
        assert 'pages/join_company.html' in template_names
        assert 'pages/companies/company_list.html' not in template_names

    def test_same_pagination_as_public_list(self):
        """Test that pagination settings match PublicCompanyListView."""
        CompanyFactory.create_batch(10)

        response = self.client.get(self.url)

        # Should have same pagination (8 items per page)
        assert len(response.context['companies']) == 8
        assert response.context['is_paginated'] is True

    def test_same_search_functionality_as_public_list(self):
        """Test that search functionality is identical to PublicCompanyListView."""
        CompanyFactory(name="Search Test Corp", industry="Testing")

        # Test name search
        response = self.client.get(self.url, {'q': 'Search Test'})
        companies = list(response.context['companies'])
        assert len(companies) == 1

        # Test industry search
        response = self.client.get(self.url, {'q': 'Testing'})
        companies = list(response.context['companies'])
        assert len(companies) == 1

    def test_context_data_structure_matches_public_list(self):
        """Test that context data structure is identical to PublicCompanyListView."""
        user = UserFactory(workplace=None)
        self.client.force_login(user)

        response = self.client.get(self.url)

        # Should have same context keys as PublicCompanyListView
        assert 'companies' in response.context
        assert 'pending_requests' in response.context
        assert 'page_obj' in response.context or not response.context['is_paginated']
