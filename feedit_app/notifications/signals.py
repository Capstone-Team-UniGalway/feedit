...
from requests.models import Request, RequestReply  # Django app name, not the Python requests library
    ...
    if created and instance.company and instance.company.employer:
        ...
        if instance.user != instance.company.employer:
            ...
            Notification.objects.create(
                ...
                message=f"New review by {reviewer_name} for {instance.company.name}: {instance.rating}/5",
                action_url=reverse('companies:detail', kwargs={'pk': instance.company.pk})
            )
...
def review_reply_post_save(sender, instance, created, **kwargs):
    ...
    if created and instance.review.user:
        ...
        Notification.objects.create(
            ...
            type=Notification.NotificationType.NEW_REVIEW,
            message=f"{instance.employer.get_full_name()} replied to your review of {instance.review.company.name}",
            action_url=reverse('companies:detail', kwargs={'pk': instance.review.company.pk})
        )
    ...
    if created:
        if instance.type == 'join' and instance.company and instance.company.employer:
            ...
            Notification.objects.create(
                ...
                message=f"{instance.author.get_full_name()} requested to join {instance.company.name}",
            ...
            for user in User.objects.filter(is_superuser=True, is_active=True):
                Notification.objects.create(
                    ...
                    message=f"{instance.author.get_full_name()} requested to claim {instance.company.name}",
                    action_url=instance.get_absolute_url()
                )
        elif instance.type == 'other' and instance.company and instance.company.employer:
            # Notify the company employer about the general request
            Notification.objects.create(
                recipient=instance.company.employer,
                type=Notification.NotificationType.GENERAL_REQUEST,
                message=f"{instance.author.get_full_name()} submitted a request: {instance.title}",
                action_url=instance.get_absolute_url()
            )
    # For status updates (not new requests)
    elif not created and kwargs.get('update_fields') and 'status' in kwargs.get('update_fields'):
            ...
            if instance.author:
                if instance.type == 'join':
                    Notification.objects.create(
                        ...
                        message=f"Your request to join {instance.company.name} has been approved!",
                        action_url=instance.get_absolute_url()
                    )
                elif instance.type == 'claim':
                    Notification.objects.create(
                        ...
                        message=f"Your request to claim {instance.company.name} has been approved!",
                        action_url=instance.get_absolute_url()
                    )
                else:
                    Notification.objects.create(
            ...
            if instance.author:
                if instance.type == 'join':
                    Notification.objects.create(
                        recipient=instance.author,
                        type=Notification.NotificationType.JOIN_RESPONSE,
                        message=f"Your request to join {instance.company.name} has been rejected.",
                        action_url=instance.get_absolute_url()
                    )
                elif instance.type == 'claim':
                    Notification.objects.create(
                        recipient=instance.author,
                        type=Notification.NotificationType.CLAIM_RESPONSE,
                        message=f"Your request to claim {instance.company.name} has been rejected.",
                        action_url=instance.get_absolute_url()
                    )
                else:
                    Notification.objects.create(
...
def request_reply_post_save(sender, instance, created, **kwargs):
    ...
    if created:
        ...
        recipient = request_obj.author if instance.author != request_obj.author else request_obj.company.employer
        ...
        if recipient:
            ...
            Notification.objects.create(
                ...
                message=f"{instance.author.get_full_name()} replied to your request: {request_obj.title}",
